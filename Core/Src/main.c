...
void LED_Task(void const * argument)
{
	extern uint8_t blinkFrequency;

	    while (1) {
	        switch (blinkFrequency) {
	            case 0: // LED desligado
	                HAL_GPIO_WritePin(BLINK_LED_GPIO_Port, BLINK_LED_Pin, GPIO_PIN_SET);
	                osDelay(100); // Pequeno atraso para evitar alto consumo de CPU
	                break;
	            case 1: // Piscar a 1 Hz
	                HAL_GPIO_TogglePin(BLINK_LED_GPIO_Port, BLINK_LED_Pin);
	                osDelay(500); // 500 ms on/off
	                break;
	            case 2: // Piscar a 10 Hz
	                HAL_GPIO_TogglePin(BLINK_LED_GPIO_Port, BLINK_LED_Pin);
	                osDelay(50); // 50 ms on/off
	                break;

	            case 3: // Piscar a 20 Hz
					HAL_GPIO_TogglePin(BLINK_LED_GPIO_Port, BLINK_LED_Pin);
					osDelay(25); // 50 ms on/off
					break;
	        }
	    }
...
void Button_Task(void const * argument)
{
	static uint8_t lastButtonState = 0;
	    ...
	    while (1) {
	        uint8_t currentState = HAL_GPIO_ReadPin(PUSH_BUTTON_GPIO_Port, PUSH_BUTTON_Pin);
	        if (currentState == GPIO_PIN_SET && lastButtonState == GPIO_PIN_RESET) {
	            // Botão pressionado - altera a frequência do LED
	            blinkFrequency = (blinkFrequency + 1) % 4; // Alterna entre 0 (apagado), 1 (1 Hz) e 2 (10 Hz)
	        }
	        lastButtonState = currentState;
	        osDelay(50); // Debounce do botão
	    }
}
